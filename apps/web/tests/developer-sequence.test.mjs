...
test("unlocks one freely scrollable question set for every role", async () => {
  ...
  assert.match(source, /const mobileTouchScrollSpeed = 1\.6/);
  assert.match(source, /const mobileTouchInertiaTimeConstantMs = 220/);
  assert.match(source, /const mobileTouchMaximumVelocity = 2\.4/);
  ...
  assert.match(source, /addEventListener\("touchstart", onTouchStart, \{ capture: true, passive: true \}\)/);
  assert.match(source, /addEventListener\("touchmove", onTouchMove, \{ capture: true, passive: false \}\)/);
  assert.match(source, /addEventListener\("touchend", onTouchEnd, \{ capture: true, passive: true \}\)/);
  assert.match(source, /event\.touches\.length !== 1/);
  assert.match(source, /verticalGesture = Math\.abs\(totalY\) > Math\.abs\(totalX\)/);
  assert.match(source, /const routedDelta = \(lastTouchY - touch\.clientY\) \* mobileTouchScrollSpeed/);
  assert.match(source, /scrollElement\.scrollTop \+= routedDelta/);
  assert.match(source, /touchVelocity \*= Math\.exp\(-elapsed \/ mobileTouchInertiaTimeConstantMs\)/);
  assert.match(source, /prefers-reduced-motion: reduce/);
